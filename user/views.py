...
def find_location(frip_id):
    return SubRegion.objects.filter(fripsubregion__frip_id=frip_id)

class SignUpView(View):
    VALIDATION_RULES = {
        'email'   : lambda email   : not re.match(EMAIL_REGEX, email),
        'password': lambda password: not re.match(PASSWORD_REGEX, password)
    }
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse({"message": "EMAIL_ALREADY_EXIST"}, status=401)
        except KeyError:
...
class KakaoView(View):
    def post(self, request):
        ...
        kakao_user_data  = user_request.json()
        user_information = kakao_user_data['properties']
        kakao_id         = kakao_user_data['id']
        nickname         = user_information.get('nickname', None)

        user, _      = User.get_or_crate(kakao_id = kakao_id, ...)
...
class LikeView(View):
    ...
    def post(self,request):
        try:
            ...
            if UserFripLike.objects.filter(user_id=user_id, frip_id=frip_id).exists():
                UserFrip.objects.filter(
                    user_id  = request.user.id,
                    frip_id  = data['frip_id']
                ).delete()
            else:
                UserFripLike.objects.create(
                    user_id  = request.user.id,
                    frip_id = data['frip_id']
            )
                ...
                "data" : [
                    {
                        "id"   : detail.id,
                        "name" : detail.name
                    } for detail in InterestDetail.objects.filter(interest_id=interest)
                ]
        ...
        frip_list     = [
                 {
                  ...
                  "new"          : interestfrip.frip.created_at >= datetime.timedelta(days = 60),
         ...
         frip_list=[
                 {
                  ...
                  "new":True if purchasefrip.frip.id in is_new else False, # <== same as above
